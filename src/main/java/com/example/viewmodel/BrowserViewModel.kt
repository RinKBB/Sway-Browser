            ...
            dao.getAllTabs().first().let { currentTabs ->
                if (currentTabs.isEmpty()) {
                    val defaultTabs = listOf(
                        TabEntity("1", "Sway Browser — Главная", savedHome, isIncognito = false, isActive = true, isBrowsing = false, historyIndex = 0, historyStackJson = "[{\"url\":\"$savedHome\",\"title\":\"Sway Browser — Главная\"}]"),
