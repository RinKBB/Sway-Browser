...
[📥 Скачать APK](#installation) • [🏗️ Архитектура](#architecture) • [🛠️ Стек технологий](#tech-stack) • [🎯 Функционал](#features) • [🔧 Разработка](#development) • [❓ FAQ](#faq)
...
**Sway Browser** — это полнофункциональный мобильный браузер для Android, разработанный на современном стеке Jetpack Compose с фокусом на удобство и производительность.
...
# Создайте .env файл с API к��ючом (см. Development раздел)
...
Проект использует **MVVM архитектуру** с четким разделением ответственности и реактивным потоком данных через StateFlow. Это обеспечивает масштабируемость и тестируемость кода.
...
- 🚀 **Умный fallback:** если сеть недоступна или API недоступен, возвращает кеш предварительно загруженных тематических фотографий
...
Или, как минимум, это выглядит так. Возможно, это космонавт, застрявший в текстурах. Или, скорее всего, это просто две сферы, которые AI решил назвать "S".

**Мы могли бы выбросить это.** Но вместо этого... мы оставили.

Потому что в этой случайности есть совершенство. В этом отказе от контроля есть правда. В этих двух колобках нет попытки умничать — только чистая, неподдельная абсурдность.

**✨ Таким образом, "два колобка на зубочистке" стали официальным талисманом Sway Browser — символом того, что иногда самые лучшие идеи рождаются не из планирования, а из полного непонимания.**
...
**👉 [Скачать Latest Release на GitHub Releases](https://github.com/RinKBB/Sway-Browser/releases)**

Просто загрузите `.apk` файл и установите его на своё устройство.

### Пошаговая установка

1. **Перейдите на страницу [Releases](https://github.com/RinKBB/Sway-Browser/releases)**
...
### Q: Почему браузер скачивает только медиафайлы, а не весь контент?
**A:** Sway Browser специализирован на загрузке медиа (изображения, видео) с веб-страниц. Это не полноценный браузер для сохранения всех типов контента, а утилита для удобной загрузки визуального контента.

### Q: Могу ли я использовать Sway Browser вместо Chrome/Firefox?
**A:** Да, это полноценный браузер с поддержкой большинства веб-стандартов. Однако некоторые сложные веб-приложения могут работать нестабильно. Протестируйте перед тем, как использовать как основной браузер.

### Q: Где хранятся мои закладки и история?
**A:** Все данные хранятся локально на устройстве в базе данных Room. Они не синхронизируются с облаком и не отправляются никуда.

### Q: Как получить API ключи для Unsplash и Gemini?
**A:** Смотрите раздел [Локальная разработка](#development). Оба сервиса предоставляют бесплатные ключи для разработки.

### Q: Почему некоторые изображения не загружаются?
**A:** Возможные причины:
- Сайт блокирует загрузки с User-Agent браузера
- Изображение защищено CORS
- Нет подключения к интернету
- API лимиты исчерпаны

### Q: Можно ли установить приложение через Google Play?
**A:** В настоящее время нет. Приложение распространяется только через GitHub Releases для избежания комиссии Google Play.
